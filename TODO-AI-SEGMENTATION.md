...
Goal: Integrate AI-assisted segmentation into MaskTap using ONNX SAM / EfficientViT models.
	...
	•	Lite demo model support anticipated for future (small model bundled with app).
...
export interface SegmentationModel {
  ...
  /** Decode embeddings for given points and mask mode */
  decode(
    embeddingKey: string,
    points: Point[],
...
type DecoderMessage =
  | { type: "LOAD_MODEL"; modelId: string; file: ArrayBuffer }
  | { type: "DECODE"; embeddingKey: string; points: Point[]; mode: "add" | "subtract" | "replace" }
  | { type: "DISPOSE" }
  | { type: "CANCEL_DECODE" };
...
type Mask = {
  ...
  alpha: Uint8Array; // 0–255
	...
	•	Binary masks created centrally with threshold (alpha >= 128).
...
10. Cold Start & Lite Demo
	•	Initial download prompt with file size.
	•	Lite demo model (small bundled model) anticipated for first-time users; not implemented yet, but code should allow default model selection.
...
/ai-segmentation/       <-- top-level library folder
  /core/
    /workers/
      encoderWorker.ts
      decoderWorker.ts
      protocol.ts
    /utils/
      mask-utils.ts       # alpha → binary, ImageData
...
14. Future Considerations
	...
	•	Coordinate mapping pipeline (image → model → canvas) critical.

⸻

This plan is now developer-ready: it defines the interface, worker architecture, storage, UI integration, mask handling, model sources, file structure, runtime choices, click handling, and future-proofing.
